				...
				int link_id)
{
	...
	if (link_id)
		bit = BIT(link_id);
	else if (bes_fail(notify_cmd != STA_NOTIFY_AWAKE, "notify_cmd"))
		bit = 0;
	else
...
static int bes2600_set_tim_impl(struct bes2600_vif *priv, bool aid0_bit_set)
{
	...
	bes_fail(wsm_update_ie(hw_priv, &update_ie, priv->if_id), "wsm_update_ie");
...
void bes2600_set_cts_work(struct work_struct *work)
{
	...
	the same ERP values */
	bes_fail(wsm_write_mib(hw_priv, WSM_MIB_ID_NON_ERP_PROTECTION,
...
static int __maybe_unused bes2600_set_btcoexinfo(struct bes2600_vif *priv)
{
	...
	if (priv->mode == NL80211_IFTYPE_STATION) {
		...
		bes_fail(bes2600_upload_pspoll(priv), "bes2600_upload_pspoll");
		bes_fail(bes2600_upload_null(priv), "bes2600_upload_null");
	} else {
		return 0;
	}
	...
	ret = bes_fail(wsm_write_mib(cw12xx_vifpriv_to_hwpriv(priv),
		WSM_MIB_ID_OVERRIDE_INTERNAL_TX_RATE,
		&arg, sizeof(arg), priv->if_id), "wsm_write_mib");

	return ret;
}

// All bes2600_bss_info_changed_* functions are called only by bes2600_bss_info_changed()
static void bes2600_bss_info_changed_bssid(struct bes2600_vif *priv,
...
static void bes2600_bss_info_changed_arp_filter(struct ieee80211_hw *dev,
	struct ieee80211_vif *vif,
	struct bes2600_vif *priv,
...
static void bes2600_bss_info_changed_beacon(struct bes2600_vif *priv,
	struct ieee80211_bss_conf *info)
{
	/* Beacon content / template update logic */
...
#endif
	bes_fail(bes2600_upload_beacon(priv), "bes2600_upload_beacon");
	bes_fail(bes2600_update_beaconing(priv), "bes2600_update_beaconing");
}

static void bes2600_bss_info_changed_beacon_enabled(struct bes2600_vif *priv,
	struct ieee80211_bss_conf *info)
{
	bes_devel("BSS_CHANGED_BEACON_ENABLED dummy: %d\n", info->enable_beacon);
	priv->enable_beacon = info->enable_beacon;
}

static void bes2600_bss_info_changed_beacon_int(struct bes2600_vif *priv,
	struct ieee80211_bss_conf *info)
{
	bes_devel("BSS_CHANGED_BEACON_INT: %d\n", info->beacon_int);
	/* Restart AP only when connected */
	if (priv->join_status == BES2600_JOIN_STATUS_AP)
			bes_fail(bes2600_update_beaconing(priv), "bes2600_update_beaconing");
}

static void bes2600_bss_info_changed_assoc(struct bes2600_vif *priv,
	struct bes2600_common *hw_priv,
	struct ieee80211_bss_conf *info,
	struct ieee80211_vif_cfg *cfg)
{
...
static void bes2600_bss_info_changed_rates_and_ht(struct bes2600_vif *priv,
	...
	struct ieee80211_vif_cfg *cfg)
{
	bes_devel("BSS_CHANGED rates/ERP/HT\n");

	/* Handle basic rates, ERP preamble, HT capabilities, etc. */
	if (changed & BSS_CHANGED_BASIC_RATES)
		bes_devel("Basic rates changed\n");

	if (changed & BSS_CHANGED_ERP_PREAMBLE)
		bes_devel("ERP preamble changed\n");

	if (changed & BSS_CHANGED_HT)
		bes_devel("HT capabilities changed\n");
	...
	if (cfg->assoc) { /* TODO: ibss_joined */
			...
			if (sta) {
					...
					bes_fail(wsm_write_mib(hw_priv,
											WSM_MID_ID_SET_HT_PROTECTION,
											&val, sizeof(val), priv->if_id), "wsm_write_mib");
			}
			...
			bes_devel("[STA] Preamble: %d, " \
							...
							priv->bss_params.operationalRateSet,
							priv->association_mode.basicRateSet);
			bes_fail(wsm_set_association_mode(hw_priv,
									&priv->association_mode, priv->if_id), "wsm_set_association_mode");
			/*
			...
			bes_fail(wsm_keep_alive_period(hw_priv,
									BES2600_KEEP_ALIVE_PERIOD /* sec */,
									priv->if_id), "wsm_keep_alive_period");
			bes_fail(wsm_set_bss_params(hw_priv, &priv->bss_params,
									priv->if_id), "wsm_set_bss_params");
			priv->setbssparams_done = true;
			bes_fail(wsm_set_beacon_wakeup_period(hw_priv,
			...
			if (priv->vif->p2p) {
					bes_devel(
									...
									"configuration.\n");
					bes_fail(wsm_set_p2p_ps_modeinfo(hw_priv,
											&priv->p2p_ps_modeinfo, priv->if_id), "wsm_set_p2p_ps_modeinfo");
#if defined(CONFIG_BES2600_USE_STE_EXTENSIONS)
...
static void bes2600_bss_info_changed_assoc_and_cts(struct bes2600_vif *priv,
	struct bes2600_common *hw_priv,
	struct ieee80211_bss_conf *info)
{
...
static void bes2600_bss_info_changed_assoc_and_erp(struct bes2600_vif *priv,
	struct bes2600_common *hw_priv,
	struct ieee80211_bss_conf *info)
{
	...
	bes_fail(wsm_write_mib(hw_priv, WSM_MIB_ID_DOT11_SLOT_TIME,
		&slot_time, sizeof(slot_time), priv->if_id), "wsm_write_mib");
	...
	struct ieee80211_bss_conf *info)
{
	...
	if (info->cqm_rssi_thold || info->cqm_rssi_hyst) {
		...
		if (priv->cqm_use_rssi) {
			 ...
			 * RCPI = (RSSI + 110) * 2 */
			threshold.upperThreshold =
				(info->cqm_rssi_thold + info->cqm_rssi_hyst + 110) * 2;
	...
	bes_fail(wsm_set_rcpi_rssi_threshold(hw_priv, &threshold,
							priv->if_id), "wsm_set_rcpi_rssi_threshold");
	...
	if (priv->cqm_beacon_loss_count != info->cqm_beacon_miss_thold) {
		...
		if (priv->bss_params.aid) {
			bes_fail(wsm_set_bss_params(hw_priv, &priv->bss_params, priv->if_id), "wsm_set_bss_params");
			priv->setbssparams_done = true;
		}
	}
#endif /* CONFIG_BES2600_USE_STE_EXTENSIONS */
...
static void bes2600_bss_info_changed_bandwidth(struct bes2600_vif *priv,
	struct bes2600_common *hw_priv,
	struct ieee80211_bss_conf *info,
	struct ieee80211_vif_cfg *cfg)
{
...
#if defined(CONFIG_BES2600_USE_STE_EXTENSIONS)
static void bes2600_bss_info_changed_p2p_ps(struct bes2600_vif *priv,
	struct ieee80211_bss_conf *info)
{
...
#define NOA_DELAY_START_MS	300
	...
	print_hex_dump_bytes("p2p_set_ps_modeinfo: ",
		DUMP_PREFIX_NONE, (u8 *)modeinfo, sizeof(*modeinfo));

	if (priv->join_status == BES2600_JOIN_STATUS_STA ||
		priv->join_status == BES2600_JOIN_STATUS_AP) {

		bes_fail(wsm_set_p2p_ps_modeinfo(hw_priv, modeinfo, priv->if_id), "wsm_set_p2p_ps_modeinfo");
	}
	...
	u64 changed)
{
	...
	if (changed & BSS_CHANGED_BSSID)
		bes2600_bss_info_changed_bssid(priv, info);

	if (changed & BSS_CHANGED_ARP_FILTER)
		bes2600_bss_info_changed_arp_filter(dev, vif, priv, hw_priv, info, conf, cfg);

	if (changed & BSS_CHANGED_BEACON)
		bes2600_bss_info_changed_beacon(priv, info);

	if (changed & BSS_CHANGED_BEACON_ENABLED)
		bes2600_bss_info_changed_beacon_enabled(priv, info);

	if (changed & BSS_CHANGED_BEACON_INT)
		bes2600_bss_info_changed_beacon_int(priv, info);

	if (changed & BSS_CHANGED_ASSOC)
		bes2600_bss_info_changed_assoc(priv, hw_priv, info, cfg);
	...
	if (changed & (
		...
		bes2600_bss_info_changed_rates_and_ht(priv, hw_priv, info, changed, vif, cfg);

	if (changed & (
		BSS_CHANGED_ASSOC | BSS_CHANGED_ERP_CTS_PROT))
		bes2600_bss_info_changed_assoc_and_cts(priv, hw_priv, info);


	if (changed & (
		BSS_CHANGED_ASSOC | BSS_CHANGED_ERP_SLOT))
		bes2600_bss_info_changed_assoc_and_erp(priv, hw_priv, info);

	if (changed & (BSS_CHANGED_ASSOC | BSS_CHANGED_CQM))
		bes2600_bss_info_changed_assoc_and_cqm(priv, hw_priv, info);
	...
	if (changed & BSS_CHANGED_P2P_PS)
		bes2600_bss_info_changed_p2p_ps(priv, info);
#endif /* CONFIG_BES2600_USE_STE_EXTENSIONS */
		...
		*(__le16 *)frame.skb->data =
		...
		if (0 /* priv->vif->p2p */)
			...
			ret = wsm_set_template_frame(hw_priv, &frame,
					priv->if_id);
			bes_fail(wsm_set_probe_responder(priv, false), "wsm_set_probe_responder");
		}
	}
	dev_kfree_skb(frame.skb);
...
static int bes2600_start_ap(struct bes2600_vif *priv)
{
	...
	ret = bes_fail(wsm_start(hw_priv, &start, priv->if_id), "wsm_start");
	if (!ret)
	...
	if (!ret) {
		...
		bes_fail(wsm_set_block_ack_policy(hw_priv,
			BES2600_TX_BLOCK_ACK_DISABLED_FOR_ALL_TID,
			BES2600_RX_BLOCK_ACK_DISABLED_FOR_ALL_TID, priv->if_id), "wsm_set_block_ack_policy"); */
	...
	if (priv->mode == NL80211_IFTYPE_AP) {
		...
		if (priv->join_status != BES2600_JOIN_STATUS_AP ||
			...
			if (priv->join_status != BES2600_JOIN_STATUS_PASSIVE)
				bes_fail(wsm_reset(hw_priv, &reset,
						  priv->if_id), "wsm_reset");
			priv->join_status = BES2600_JOIN_STATUS_PASSIVE;
			bes_fail(bes2600_start_ap(priv), "bes2600_start_ap");
			wsm_unlock_tx(hw_priv);
		} else
...
void bes2600_link_id_gc_work(struct work_struct *work)
{
	...
	for (i = 0; i < CW1250_MAX_STA_IN_AP_MODE; ++i) {
		...
		if (priv->link_id_db[i].status == BES2600_LINK_RESERVE ||
			...
			spin_unlock_bh(&priv->ps_state_lock);
			if (need_reset) {
				bes_fail(cw12xx_unmap_link(priv, i + 1), "cw12xx_unmap_link");
			}
			map_link.link_id = i + 1;
			bes_fail(wsm_map_link(hw_priv, &map_link, priv->if_id), "wsm_map_link");
			...
			if (status == BES2600_LINK_RESET_REMAP) {
				memcpy(map_link.mac_addr,
						...
						ETH_ALEN);
				map_link.link_id = i + 1;
				bes_fail(wsm_map_link(hw_priv, &map_link,
						     priv->if_id), "wsm_map_link");
				next_gc = min(next_gc,
					      BES2600_LINK_ID_GC_TIMEOUT);
				priv->link_id_db[i].status =
...
void bes2600_notify_noa(struct bes2600_vif *priv, int delay)
{
	...
	if (!bes_fail(wsm_get_p2p_ps_modeinfo(hw_priv, modeinfo), "wsm_get_p2p_ps_modeinfo")) {
