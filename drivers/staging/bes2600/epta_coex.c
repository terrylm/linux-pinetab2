...
static DEFINE_SPINLOCK(coex_lock); /* Added for global variable protection */
...
int coex_set_epta_params(struct bes2600_common *hw_priv, int wlan_duration, int bt_duration, int hw_epta_enable)
{
	...
	if (wlan_duration < 10 || wlan_duration > 100000 || bt_duration < 0 || bt_duration > 100000) {
		bes_devel("invalid durations: wlan=%d, bt=%d\n", wlan_duration, bt_duration);
		return -EINVAL;
	}

	spin_lock_irqsave(&coex_lock, flags);
	...
	if (wlan_duration_cfg == wlan_duration && bt_duration_cfg == bt_duration && hw_epta_enable_cfg == hw_epta_enable) {
		...
		spin_unlock_irqrestore(&coex_lock, flags);
		return 0;
	}
	...
	if (epta_freeze_bitmap) {
		...
		*/
		if (wlan_duration < epta_freezed_wlan_duration_cfg) {
			...
			if (bt_duration < 0) { /* Prevent underflow */
				bt_duration = 0;
			}
		}
	}
...
static int coex_epta_freeze_update(struct bes2600_common *hw_priv)
{
	...
	bes_devel("%s, %d", __func__, hw_epta_enable_cfg);
	...
	if (msg.bt_duration < 0) { /* Prevent underflow */
		msg.bt_duration = 0;
	}
...
static int coex_epta_freeze(struct bes2600_common *hw_priv, int wlan_duration, uint32_t type)
{
	...
	if (wlan_duration < 20 || wlan_duration > 100000) { /* Added bounds check */
		bes_devel("invalid wlan_duration:%d\n", wlan_duration);
		return -EINVAL;
	}
...
static int coex_epta_set_connect(struct bes2600_common *hw_priv, int wlan_duration, int bt_duration, int epta)
{
	...
	if (wlan_duration < 10 || wlan_duration > 100000 || bt_duration < 0 || bt_duration > 100000) { /* Added bounds check */
		bes_devel("invalid durations: wlan=%d, bt=%d\n", wlan_duration, bt_duration);
		return -EINVAL;
	}
	...
	if (g_coex_mode & WIFI_COEX_MODE_FDD_BIT) {
		if (g_coex_mode & WIFI_COEX_MODE_FDD_HYBRID_BIT) {
			...
			if (connect_status == EPTA_STATE_WIFI_CONNECTED) {
				...
				if (hw_priv->channel && hw_priv->channel->band == NL80211_BAND_2GHZ) /* Added NULL check */
...
bool coex_is_wifi_inactive()
{
	...
	spin_lock_irqsave(&coex_lock, flags);
	bes_devel("%s, epta_conn_state:%d", __func__, epta_conn_state);
	ret = epta_conn_state == EPTA_STATE_WIFI_DISCONNECTED;
	spin_unlock_irqrestore(&coex_lock, flags);
	return ret;
}
...
void coex_set_fdd_mode(bool fdd_mode)
{
	...
	if (g_coex_mode == 0 || // tdd
		...
		bes_devel("%s, %d", __func__, fdd_mode);
...
bool coex_is_fdd_mode()
{
	...
	spin_lock_irqsave(&coex_lock, flags);
	bes_devel("%s, %d", __func__, coex_fdd_mode);
	ret = coex_fdd_mode;
	spin_unlock_irqrestore(&coex_lock, flags);
	return ret;
}
...
bool coex_is_bt_a2dp()
{
	...
	spin_lock_irqsave(&coex_lock, flags);
	bes_devel("%s, coex_bt_state:%d", __func__, coex_bt_state);
	ret = coex_bt_state == 3;
	spin_unlock_irqrestore(&coex_lock, flags);
	return ret;
}
